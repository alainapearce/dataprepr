...
#'  \itemize{
#'     \item{For base_zero = TRUE: 0 = Extremely Untrue; 1 = Quite Untrue; 2 = Sightly Untrue; 3 = Neither True nor False; 4 = Slightly True; 5 - Quite True; 6 - Extremely True, pna_value (param) = does not apply }
#'     \item{For base_zero = FALSE: 1 = Extremely Untrue; 2 = Quite Untrue; 3 = Sightly Untrue; 4 = Neither True nor False; 5 = Slightly True; 6 - Quite True; 7 - Extremely True, pna_value (param) = does not apply }
#'   }
...
#' cbq_score_data <- score_cbq(cbq_data, base_zero = TRUE, id = 'ID', pna_value = 7)
#' 
#' # scoring for the cbq with IDs, when cbq values range from 1-7, and "does not apply" responses are scored as NA
#' cbq_score_data <- score_cbq(cbq_data, base_zero = FALSE, id = 'ID')
#' 
#' \dontrun{
#' }
#'
...
score_cbq <- function(cbq_data, pna_value, base_zero = TRUE, id, session_id, extra_scale_cols = c()) {
  ...
  if (isTRUE(methods::hasArg(pna_value))) {
    ...
    cbq_data_edit[cbq_items] <- lapply(cbq_data_edit[cbq_items], function(x) ifelse(x == pna_value, NA, x))
  ...
  if (isTRUE(base_zero)){
    if (min < 0 | max > 6) {
      warning("range in CBQ data (excluding pna_value) is outside expected range given base_zero = TRUE (expected range: 0-6). Scoring may be incorrect")
    } 
  } else {
    if (min < 1 | max > 7) {
      warning("range in CBQ data (excluding pna_value) is outside expected range given base_zero = FALSE (expected range: 1-7). Scoring may be incorrect")
    } 
  }
