...
test_data_base0 <- data.frame(matrix(nrow = 0, ncol = length(column_names)))

# add columns names
colnames(test_data_base0) <- column_names
...
test_data_base0[1,c("ID")] <- "sub-001" 
test_data_base0[1,dd_items] <- 0 
test_data_base0[1,ef_items] <- 1
test_data_base0[1,eoe_items] <- 2
test_data_base0[1,eue_items] <- 3
test_data_base0[1, ff_items_forwardscore] <- 0
test_data_base0[1, ff_items_reversescore] <- 4 # reverse scores from score_cebq() should be 0
test_data_base0[1, fr_items] <- 3
test_data_base0[1, se_items_forwardscore] <- 0
test_data_base0[1, se_items_reversescore] <- 4 # reverse scores from score_cebq() should be 0
test_data_base0[1, sr_items_forwardscore] <- 0
test_data_base0[1, sr_items_reversescore] <- 4 # reverse scores from score_cebq() should be 0

# generate data with nas
test_data_base0_nas <- test_data_base0
test_data_base0_nas[c("cebq1", "cebq2")] <- NA

# generate data base 1
test_data_base1 <- test_data_base0
test_data_base1[2:36] <- test_data_base0[2:36] + 1
...
expected_scores <-
  data.frame(
    ...
    cebq_eue = 4,
    cebq_ff = 1,
    cebq_approach = as.numeric(round(rowMeans(test_data_base0[1,approach_items]),3) + 1),
    ...
    cebq_pe = round(rowMeans(test_data_base0[1,pe_items]),3) + 1,
    cebq_ee = round(rowMeans(test_data_base0[1,ee_items]),3) + 1
  )
...
expected_scores_nas <-
  data.frame(
    ...
    cebq_eue = 4,
    cebq_ff = 1,
    cebq_approach = as.numeric(round(rowMeans(test_data_base0_nas[1,approach_items]),3) + 1),
    cebq_avoid = round(mean(c(rep(3, 4), rep(0, 6), rep(0, 4),rep(0, 5))), 3) + 1,
    cebq_rbe = as.double(NA),
    cebq_pe = round(rowMeans(test_data_base0_nas[1,pe_items]),3) + 1,
...
scored_base0 <- score_cebq(test_data_base0, base_zero = TRUE, id = "ID")
scored_base1 <- score_cebq(test_data_base1, base_zero = FALSE, id = "ID")
scored_base0_nas <- score_cebq(test_data_base0_nas, base_zero = TRUE, id = "ID")
...
test_that("bids_phenotype includes input data plus scores", {
  ...
  merged <- merge(test_data_base0, expected_scores)
  
  # note: this only works because test_data_base0 did not have underscores in col names. otherwise, colnames would be modified
...
test_that("warning if values are out of expected bounds", {
  ...
  expect_warning(score_cebq(test_data_base0, base_zero = FALSE))

  # expect warning if range is 1-5 and base_zero = TRUE
  expect_warning(score_cebq(test_data_base1, base_zero = TRUE))
...
test_that("errors if arguments don't meet expectations ", {
  ...
  expect_error(score_cebq(test_data_base0, base_zero = "FALSE"))
  
  # expect error if ID isn't in database
  expect_error(score_cebq(test_data_base0, id = "ilovecats"))
})
